...
def train_student_encoder(teacher_model, train_paths):
    student = StudentEncoder_1x16().to(DEVICE)
    ...
    torch.save(student.state_dict(), f'./AI_Real_v2/Model/student_encoder_kd_k8s_{EPOCHS}_1x16.pth')
    return student

# === Main ===
def main():
    teacher = Encoder().to(DEVICE)
    teacher.load_state_dict(torch.load(f'./AI_Real_v2/Model/cnn_contrastive_encoder_k8s_20.pth'))
