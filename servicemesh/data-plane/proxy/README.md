...
- 세션 윈도우가 차기 전에는 판정이 없어 Forward된다. 논문 Algorithm 1 line 6도 같은
  성질이다. 다만 여기서 세는 것은 패킷이 아니라 **이미지화된 패킷**이다 — east-west
  HTTP(:8080)에서 Converter는 HTTP 요청만 이미지화하고 SYN·ACK·응답은 `None`으로
  버리므로, 실제 문턱은 "같은 세션의 **요청 w개**"다. 연결마다 요청 1개씩만 보내는
  클라이언트는 판정이 영영 나오지 않는다(`.agents/handoffs/model-crit.md`의
  "keep-alive 필수"가 같은 성질을 학습 쪽에서 다룬다). 반면 `:443`·`:6443`·`:22`·
  `:9000`은 payload와 무관하게 전부 이미지화되므로 한 연결의 핸드셰이크만으로 창이
  찬다 — k1·k2가 가장 잘 걸리는 이유다.
- 위 성질 때문에 Drop은 연결을 끊지 않는다. 끊으면 클라이언트가 새 연결을 열고, 새
  연결은 세션 id가 달라 창이 0부터 다시 차 그대로 통과한다 — 집행이 스스로를 무효화한다.
  대신 403을 돌려주고 연결을 유지해 같은 세션 id에 머물게 한다. 503·429는 Apache
  HttpClient와 nginx가 재시도 대상으로 삼으므로 쓰지 않는다. 비HTTP(raw TCP)는 돌려줄
  상태 코드가 없어 종료가 유일한 집행이다.
